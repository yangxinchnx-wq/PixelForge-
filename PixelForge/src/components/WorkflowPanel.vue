...
const steps = ref<WorkflowStep[]>([
  { id: 'input', label: '描述输入', icon: 'chat', status: 'pending' },
  { id: 'parse', label: 'AI 解析', icon: 'brain', status: 'pending' },
  { id: 'generate', label: '图片生成', icon: 'sparkle', status: 'pending' },
  { id: 'postprocess', label: '后处理', icon: 'wand', status: 'pending' },
  { id: 'export', label: '导出', icon: 'download', status: 'pending' },
]);
...
// ─── Manual step click (jump to step) ─────────────────
function clickStep(_step: WorkflowStep, index: number) {
  // 仅允许点击已完成或当前步骤之前的步骤
  if (index === 0) {
    // 点击第一步：重置整个工作流
    steps.value.forEach((s) => (s.status = 'pending'));
