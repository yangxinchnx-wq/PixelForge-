...
const modalStyle = computed(() => {
  if (!hasCustomSize.value) return {} as Record<string, string>;
  return {
    position: 'absolute',
  ...
  function onMove(ev: MouseEvent) {
    ...
    if (corner === 'tl' || corner === 'bl') {
      ...
      if (newWidth > MAX_WIDTH) {
        newLeft = startLeft + (startWidth - MAX_WIDTH);
        newWidth = MAX_WIDTH;
      }
    } else {
      newWidth = startWidth + dx;
    ...
    if (corner === 'tl' || corner === 'tr') {
      ...
      if (newHeight > MAX_HEIGHT) {
        newTop = startTop + (startHeight - MAX_HEIGHT);
...
<template>
  <div v-if="internalVisible" ref="overlayRef" class="pf-modal-overlay" @click="emit('close')">
    <div ref="modalRef" class="pf-modal pf-settings-modal" :style="modalStyle" @click.stop>
          ...
          <section v-if="activeTab === 'autosave'" class="pf-settings-section">
            ...
            <div v-if="autoSaveEnabled" class="pf-setting-card pf-setting-card-column">
              <div class="pf-setting-card-info">
                ...
                <div class="pf-setting-card-desc">变更后等待多久自动保存（{{ Math.round(autoSaveInterval / 1000) }} 秒）</div>
              </div>
      ...
      <!-- 四角拖拽缩放手柄 -->
      <div
        class="pf-resize-handle pf-resize-tl"
        title="拖拽调整大小（双击重置）"
        @mousedown="startResize('tl', $event)"
        @dblclick.prevent="resetSize"
      />
      <div
        class="pf-resize-handle pf-resize-tr"
        title="拖拽调整大小（双击重置）"
        @mousedown="startResize('tr', $event)"
        @dblclick.prevent="resetSize"
      />
      <div
        class="pf-resize-handle pf-resize-bl"
        title="拖拽调整大小（双击重置）"
        @mousedown="startResize('bl', $event)"
        @dblclick.prevent="resetSize"
      />
      <div
        class="pf-resize-handle pf-resize-br"
        title="拖拽调整大小（双击重置）"
